...
if bids is not None and asks is not None:
    ...
    # Show top 10 bids & asks
    bids_placeholder.subheader("Top Bids")
    bids_placeholder.dataframe(bids.head(10).style.format({"Price": "${:,.2f}", "Quantity": "{:,.4f}"}))

    asks_placeholder.subheader("Top Asks")
    asks_placeholder.dataframe(asks.head(10).style.format({"Price": "${:,.2f}", "Quantity": "{:,.4f}"}))
