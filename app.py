...
st.set_page_config(page_title="BTC/USDT Live Order Book", layout="wide", page_icon="📊")

SYMBOL = "BTCUSDT"
LEVELS = 30  # order book depth
...
time.sleep(1)  # give socket time to connect

st.title("📊 BTC/USDT Live Order Book (WebSocket)")
st.caption("Streaming from Binance WebSocket · Updates ~100ms")

# ---------- Main UI ----------
if latest_data["bids"] is None or latest_data["asks"] is None:
    st.warning("Waiting for live data from Binance WebSocket...")
    st.stop()
...
st.caption("Note: streaming top-of-book depth from Binance. Liquidity can vanish instantly — use as context, not financial advice.")
